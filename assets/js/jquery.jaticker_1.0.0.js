...
(function($){
	...
	JT.parseRuby = function(opt, rubyNode, parent) {
		...
		rubyNode.contents().each(function() {
			...
			if (!!origin && !!ruby) {
				let texts = JT.parseText(opt, ruby, parent);
				let word = texts[0];
	...
	$.fn.jaticker = function(options) {
		return this.each(function(){
			let opt = $.extend({}, JT.opt, options == undefined ? {} : options);
			opt.cursor = $e('span',  'jaticker-cursor').text(opt.cursorStr).css('visibility', 'hidden');
			if (opt.hideCursor) {
				opt.cursor.hide();
			}
			let jaticker = new JT.Block(opt, this);
	...
	$.fn.jatickerStart = function(options) {
		return this.each(function(){
			let jaticker = $(this).data('jaticker');
			if (!!jaticker) {
				jaticker.start();
			}
		});
